...
{
	int i;
   #pragma omp parallel for num_threads(30) firstprivate(n, xr, yr, arrayX, arrayY) private(i)
	for(i = 0; i < n; i++){
...
#pragma omp parallel for num_threads(30) firstprivate(n, Ones,arrayX, arrayY, index, Y, Z, iter, max_size, probability) private(i, j, index_X, index_Y)
   	for(i = 0; i<n; i++){
   		for(j = 0; j < Ones; j++){
...
#pragma omp parallel for num_threads(30) firstprivate(n, arrayX, arrayY, weights) private(i) reduction(+:estimate_x, estimate_y)
	for(i = 0; i < n; i++){
...
{
	int i;
#pragma omp parallel for num_threads(30) firstprivate(n, u, u1) private(i)
	for(i = 0; i < n; i++){
   		u[i] = u1 + i/((double)(n));
   	}
}
...
#pragma omp parallel for num_threads(30) firstprivate(n, cfd, u, arrayX, arrayY, x_j, y_j) private(i, j)
	for(j = 0; j < n; j++){
...
#pragma omp parallel for num_threads(30) firstprivate(n, arrayX, arrayY, x_j, y_j, weights) private(i)
	for(i = 0; i < n; i++){
		arrayX[i] = x_j[i];
		arrayY[i] = y_j[i];
		weights[i] = 1/((double)(n));
